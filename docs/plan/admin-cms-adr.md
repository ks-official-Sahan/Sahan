...
- Built, and where it differs from the plan:
  - `lib/chatbot/{knowledge,guard,session,prompts}.ts`: `guardUserMessage` reuses `wrapUserData` from step 12's `lib/ai/guard.ts` unchanged. `filterModelOutput(text, { siteHostname?, allowedHosts?, siteEmail? })` is a pure function (the caller resolves the real values from `env.SITE_URL` and `config/site.ts`'s `Site.email` and passes them in, never read from inside the function): it strips HTML, drops a secret-shaped reply outright via `looksLikeLeak`, strips every `/admin` path, and for every `http(s)`, `www.` or `mailto:` URL found checks the **hostname** (or the mail address) against an explicit allowlist (`hostname === allowed`, or a proper `.` -suffixed subdomain match against the site's own host) — never a substring test on the whole URL. `knowledge.ts` builds its context from typed `getPageContent()` results (no `as any`), `lib/collections` and active `ChatTrainingEntry` rows, cached under `chatbot:knowledge`, invalidated by the existing content/collection/post publish plans plus training-entry changes. `session.ts` upserts `ChatSession` by a client-generated `sessionId` and appends `ChatMessage` rows; lead capture creates an `Inquiry` with `source: "chatbot"` and links `ChatSession.inquiryId`.
  - `app/api/chat/route.ts`: content-type, size and origin checks and IP/session rate limiting in the same style as `app/api/contact/route.ts`, input capped at 1000 characters, only the last 10 messages sent to the model, streaming through the `ai` package already installed in step 12.
  - `components/site/chat/ChatWidget.tsx` mounts from `app/(site)/layout.tsx` only, behind a dynamic `ssr:false` import, and only when `features.chatbotEnabled` (from `getPublicSettings()`, resolved server-side and passed down, never fetched by the client) — turning the feature off removes the component from the tree, not just its visibility. `chatbot.config.enabled` separately governs whether a mounted widget answers or shows a paused state.
  - `app/admin/(panel)/chatbot/{page,conversations,training,training/new}.tsx` and `lib/actions/chatbot.ts`: conversation list needs `viewChatHistory`; training entry create/update/delete need `manageChatbot`, authorize first, audit in the same transaction, and invalidate `chatbot:knowledge`. The conversations and training list pages still use raw Tailwind classes instead of this repo's `components/admin/ui/styles.ts` conventions (`cardClass`/`tableClass`/`badgeClass`) that every other admin screen follows; functional, cosmetic debt only.
  - A first draft of the output filter allowed any URL whose *string* contained a channel keyword ("mail", "whatsapp", "contact"), which a hostname like `evil-mailer.com` or `phishing-whatsapp-clone.net` would also contain — found in review before this was trusted, and fixed to the exact-hostname check described above.
- Result: `tsc --noEmit` 0 errors, full unit suite passes (688 tests). I independently ran `filterModelOutput` against `https://evil-mailer.com`, `https://phishing-whatsapp-clone.net`, a mismatched `mailto:`, a legitimate subdomain of the site, a correct `mailto:`, an `/admin` path, and ordinary prose containing "Next.js" and "e.g.": every attack string was stripped, every legitimate one survived, and ordinary text was untouched. Not done: `db:seed-blog`-style live testing does not apply here, but a live AI provider call, a browser check of the widget, and a real lead-capture submission were not run.
...
- Built, and where it differs from the plan:
  - `docs/admin.md`: first-time setup, unlock and sign in, roles and permissions (points at the ADR's section 9 table rather than duplicating it, since it changes per step), add a user, restore a content version, rotate a secret (per-secret blast radius, e.g. rotating `AUTH_SECRET` signs everyone out and invalidates outstanding invite links, rotating `MEDIA_SIGNING_SECRET` invalidates cached signed `/media/*` URLs), a rotation checklist, Vercel environment setup (including `TRUSTED_PROXY_HOPS` for non-Vercel reverse proxies), break-glass for Redis (what fails open vs. safe-default when Redis is unreachable, and why that's a deliberate design choice, not a gap), and a short "verify everything is configured" section pointing at the new script and the existing integration-health/security-status admin widgets.
  - `scripts/check-env-example.mts`: parses the variable names out of `lib/env.ts`'s zod schema and out of `.env.example` (including commented-out `# NAME=` lines) with regex, diffs them both ways, and exits non-zero on any mismatch. An explicit `OUTSIDE_SCHEMA` set (`TRUSTED_PROXY_HOPS`, `NODE_ENV`) covers the small number of variables read directly from `process.env` in code that runs where `lib/env.ts`'s `server-only` import cannot load (`proxy.ts`). Never reads or prints a variable's value, only its name. Run: `pnpm exec tsx scripts/check-env-example.mts`.
  - `README.md`: a short "Admin panel" section pointing at `docs/admin.md`, the ADR and the security review, plus a copy-pasteable quick start.
  - `AGENTS.md` (and, through it, `CLAUDE.md`'s `@AGENTS.md` include): a new "Admin CMS, cache and security rules" section appended after the auto-generated Next-version-warning block, which the generator preserves on regeneration (it only replaces text between its own markers, confirmed by reading `node_modules/next/dist/server/lib/generate-agent-files.js`'s `upsertAgentRulesBlock`). Encodes the rules a future change in this codebase needs to keep: authorize-then-audit-in-transaction, secrets as parameters compared in constant time, `invalidate()` plus admin-page `revalidatePath`, sanitize rich text on both write and read, and that the shared origin/CSRF check already covers any new public `/api/*` route.
- Result: `pnpm exec tsx scripts/check-env-example.mts` passes (47 schema variables, all listed in `.env.example`; 2 read outside the schema, both accounted for). `tsc --noEmit` 0 errors. Nothing else in the repo changed by this step besides documentation and the one new script, so the full verification suite from step 17 (688 unit tests, `next build`) still applies unchanged.
