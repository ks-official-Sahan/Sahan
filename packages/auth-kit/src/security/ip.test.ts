...
test("on Vercel the platform header wins over the forwarded list", () => {
  assert.equal(
    clientIp(reader({ "x-vercel-forwarded-for": "203.0.113.9", "x-forwarded-for": "198.51.100.1" }), vercel),
    "203.0.113.9"
  );
});

test("on Vercel x-real-ip comes next, then the first forwarded entry", () => {
  assert.equal(clientIp(reader({ "x-real-ip": "203.0.113.5", "x-forwarded-for": "198.51.100.1" }), vercel), "203.0.113.5");
  assert.equal(clientIp(reader({ "x-forwarded-for": "198.51.100.1, 10.0.0.1" }), vercel), "198.51.100.1");
});
...
test("IPv6 is accepted and lower-cased", () => {
  assert.equal(clientIp(reader({ "x-real-ip": "2001:DB8::1" }), vercel), "2001:db8::1");
  assert.equal(clientIp(reader({ "x-forwarded-for": "2001:DB8::1" }), { TRUSTED_PROXY_HOPS: "1" }), "2001:db8::1");
});

test("junk gives unknown", () => {
  assert.equal(clientIp(reader({ "x-real-ip": "<script>", "x-forwarded-for": "198.51.100.7" }), vercel), "198.51.100.7");
  assert.equal(clientIp(reader({ "x-real-ip": "not an ip" }), vercel), UNKNOWN_IP);
